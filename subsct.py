...
def subdomains(target):
    ...
    try:
        r = requests.get(f'https://api.certspotter.com/v1/issuances?domain={target}&include_subdomains=true&expand=dns_names&expand=issuer&expand=cert', headers=headers, timeout=(5), verify=False)
    except Exception:
    ...
    try:
        r = requests.get(f'https://crt.sh/?q=%.{target}&output=json', headers=headers, timeout=(5), verify=False)
    except Exception:
